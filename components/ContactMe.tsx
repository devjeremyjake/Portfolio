...
export default function ContactMe({ pageInfo }: Props) {
	...
	const onSubmit: SubmitHandler<Inputs> = (formData) => {
		window.location.href = `mailto:devjeremyjake@gmail.com?subject=${formData.subject}&body=Hi, my name is ${formData.name}.${formData.message}`;
	};
	return (
			...
			<div className="flex flex-col space-y-4 md:space-y-5 lg:space-y-6 xl:space-y-6 2xl:space-y-10">
				...
				>
					...
					<textarea
						...
						className="contactInput"
					/>
					<button className="bg-[#AE561C] py-3 md:py-5 px-10 rounded-lg text-white font-bold text-lg">
						{' '}
						Submit
					</button>
				</form>
